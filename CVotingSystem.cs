    ...
    public class CVotingSystem : PRoConPluginAPI, IPRoConPluginInterface
    {
            ...
            lock (syncLock)
            {
                ...
                if (!thresholdMet)
                {
                    ...
                    else
                    {
                        ...
                        {
                            LogLive($"[CVotingSystem] [Live]: Checking Threshold -> Current Players: {currentPlayerCount} | Min Required: {minimumPlayersToVote} | WaitingState: {isWaitingForPlayers} | Phase: {currentPhase}");
                            lastThresholdLogTime = DateTime.Now;
                        }
                    }
                }
                ...
                {
                    if (isWaitingForPlayers || currentPhase == VotingPhase.Idle && roundStartDelayTimer == null && !hasVotedThisMatch)
                    {
                        isWaitingForPlayers = false;
                        LogLive($"[CVotingSystem] [Live]: Minimum player count requirement reached ({currentPlayerCount}/{minimumPlayersToVote}). Executing automatic !voterefresh and starting voting countdown immediately!");
        ...
        public void OnLevelLoaded(string mapFileName, string Gamemode, int roundsPlayed, int roundsTotal)
        {
            lock (syncLock)
            {
                ...
                {
                    LogLive($"[CVotingSystem] [Live]: Detected infinite map reload loop bug (Sequence: {currentMapLoadSequenceCount}). Restarting match entirely and forcing vote immediately.");
                    
                    try
                    {
                        // 1. Re-add the current map and mode to restart the match entirely from round 1/2
                        string roundsToInject = IsHeistOrSquadHeist(Gamemode) ? "2" : "1";
                        this.ExecuteCommand("procon.protected.send", "mapList.add", mapFileName, Gamemode, roundsToInject);

                        // 2. Switch immediately to the restarted map index and run it
                        ...
                        // 3. Safely remove every old map from the list except index 0 without using clear()
                        for (int i = 10; i >= 1; i--)
                        {
                            try { this.ExecuteCommand("procon.protected.send", "mapList.remove", i.ToString()); } catch { }
                        }
                    ...
                    {
                        LogLive("^1Error executing match restart recovery: " + ex.Message);
                    }

                    // 4. Reset voting states and clear messages
                    ResetVotingState(true);
                    SendGlobalChat("all voting stages has been deleted and clear");
                    
                    // 5. Force voting phase to start immediately
